...
class ImageStoryPlayerFrag : StoryPlayerFrag() {
    ...
    private fun setSlideRange(begin : Int, end : Int) {
        ...
        val storySlides = mutableListOf<Slide>()
        ...
        for(i in begin until end) {
            if(applicationContext != null && i < storySlides.size) {
                val slideDuration = MediaHelper.getAudioDuration(
                        applicationContext!!,
    ...
    private fun setSlidePic(slideNum: Int) {
        ...
        val tOverlay = if (Workspace.activePhase.phaseType == PhaseType.LEARN)
            Workspace.activeStory.slides[slideNum].getOverlayText(dispStory = false, origTitle = true)
        else Workspace.activeStory.slides[slideNum].getOverlayText(dispStory = false, origTitle = false)
